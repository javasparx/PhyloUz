...
//				System.out.println("ASDF");
    ...
    public void fastaLoad() {
        FileDialog fd = new FileDialog(PhyloWidget.ui.getFrame(), "Locate a Fasta format file.", FileDialog.LOAD);
        fd.pack();
        fd.setVisible(true);
        String directory = fd.getDirectory();
        String filename = fd.getFile();
        if (filename == null) {
            PhyloWidget.setMessage("Fasta load cancelled.");
            return;
        }
        final File f = new File(directory, filename);
        setMessage("Loading fasta...");
        new Thread() {
            public void run() {

                FastaReader fastaReader = new FastaReader(FastaReader.FastaFileContent.DNA_SEQUENCE, f.getPath());
                ...
                for (Sequence sequence : list) {
                    DNASequence dnaSequence = new DNASequence(sequence.getSequenceString());
